...
public class BidderDaoImpl implements BidderDao {
		...
		try(Connection con = DBUtil.provideConnection()) {
			...
			if(rs.next()){
				...
				if(x>0){
					...
					status = status +dao.assignTender(tenderId, vendorId,applicationId);
				}
			}
	...
	public String bidTender(String tenderId, String vendorId, String bidAmount,
		...
		BidderBean bidder = new BidderBean( vendorId, tenderId, bidAmount, bidDeadline, bidStatus);
...
//			Date biddeadline = bidder.getBidDeadline();
//			Date bidDate = new (bidder.getBidDeadline());
			
			ps.setDate(5, (java.sql.Date) bidDate);
